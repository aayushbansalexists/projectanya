...
# Function to update customer data from Ollama response
def update_customer_from_ollama(city, customers):
...
if not india_cities.empty:
    ...
    # Natural language input box for Ollama integration
    user_query = st.text_input("Tell me about your customers (e.g., 'I have 20000 customers in Mumbai')", "")
    if st.button("Process Query with Ollama") and user_query:
        try:
            # Query Ollama server (assuming running at http://localhost:11434 with gemma3:4b)
            ollama_url = "http://localhost:11434/api/generate"
            prompt = f"Parse this statement into a city name and customer count. Respond ONLY as valid JSON without any additional text, code blocks, or explanations: {{\"city\": \"CityName\", \"customers\": number}}. Do not include anything else. Statement: {user_query}"
            ...
            if response.status_code == 200 and response.text:
                ...
                if ollama_response:
                    ...
                    if json_match:
                        ...
                        if city and customers > 0:
                            update_customer_from_ollama(city, customers)
                ...
                st.error(f"Ollama server error: {response.status_code} - {response.text}")
        except json.JSONDecodeError as json_err:
            st.error(f"JSON parsing error: {json_err}. Extracted string: {json_str}")
        except Exception as e:
    ...
    # Display and edit customer data (combines Ollama updates and manual)
