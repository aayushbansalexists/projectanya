...
# Allow user to upload their CSV file
st.subheader("Upload Your India Cities CSV File")
uploaded_file = st.file_uploader("Choose a CSV file", type="csv")

if uploaded_file is not None:
    # Read the uploaded CSV
    try:
        india_cities = pd.read_csv(uploaded_file)
        # Select and rename relevant columns to match the code
    ...
    st.info("Please upload your India cities CSV file to proceed.")
    india_cities = pd.DataFrame()

if not india_cities.empty:
    st.subheader("Step 1: Select Indian Cities and Specify Customer Counts")
    # Create a list of city-state strings for multiselect
    city_options = (india_cities['city'] + ", " + india_cities['state_name']).tolist()
    
    selected_cities = st.multiselect(
        "Select Indian cities where you have customers",
        options=city_options,
        default=city_options[:5] if len(city_options) >= 5 else city_options,  # Pre-select first 5 if available
        help="Choose from cities in your uploaded CSV. Then specify customer counts below."
    )
    ...
    if uploaded_file is not None:
        st.warning("Uploaded CSV does not contain valid city data. Ensure it has columns: city, state, population, latitude, longitude.")
...
if st.button("Calculate Ideal Location") and not india_cities.empty and not user_customers_df.empty:
    if potential_sites:
        ...
        if centroid_lat is not None:
            ...
            map_center = [20.5937, 78.9629]  # Approximate geographic center of India
    
    # Create interactive map centered on best/centroid location
    ...
    # Add markers for potential sites (if provided)
    for site in potential_sites:
    ...
    # Add marker for centroid if computed
    if centroid_lat is not None:
        folium.Marker(
    ...
    st.info("Upload your CSV, select cities, specify customer counts, and (optionally) input sites. Then click the button to calculate.")
